...
with tab1:
    ...
    res = mycol.find_one({"username": str1, "password": str2})
    if res:
        col_left, col_right = st.columns([1, 2])
        with col_left:
            st.markdown(f"**Username:** {res.get('username', '')}")
            st.markdown(f"**Password:** {res.get('password', '')}")
        with col_right:
            st.markdown(f"**Mobile:** {res.get('mobile', '')}")
            st.markdown(f"**Email:** {res.get('email', '')}")
            st.markdown(f"**Date of Birth:** {res.get('dob', '')}")
    else:
...
with tab3:
    ...
    if st.button("Delete Account"):
        mycol.delete_one({"username": st.session_state['username']})
